...
| `SettingsPage.razor`       | Renders the select, cascades the initial theme, posts to `/api/theme` on change. |
...
browser → server: GET /  (Cookie: theme=dark)
                 ...
                 SettingsPage mounts the select with Value="dark" — no flicker
```

When the user changes themes, the select's `OnChange` POSTs to
