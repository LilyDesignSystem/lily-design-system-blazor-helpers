...
`TextSizePicker` mounts identically; only the required parameters
...
cut.Find("div.theme-picker").FocusOut();    // swallowed — the component's own move
cut.Find("div.theme-picker").FocusOut();    // the real departure
...
Assert.Empty(cut.FindAll(".theme-picker-icon"));   // default glyph replaced
Assert.Equal(2, cut.FindAll("li[role='option']").Count);  // options untouched
```

`TextSizePicker`'s `ChildContent` behaves the same way: it replaces the
