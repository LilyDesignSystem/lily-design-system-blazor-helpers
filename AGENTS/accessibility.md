...
`TextSizePicker` joined this shape last; before that it was a real
...
locale-picker's per-option `lang` — cannot be broken by a consumer
...
`LocalePicker` and `TextSizePicker` assert identically; only the
