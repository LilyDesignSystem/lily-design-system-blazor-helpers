...
No change to the public API, rendered markup (class names, ids, ARIA
attributes), or keyboard contract — the full existing test suite
(39 tests) passes unchanged. `Listbox` in this catalog was built from
scratch (unlike the JS catalogs, it had no prior keyboard logic to
extend) and deliberately does not query a live DOM for its options —
Blazor has no synchronous DOM access without JS interop — instead
taking `OptionCount`/`OptionId`/`OptionLabel` as parameters, mirroring
this component's own pre-refactor `ActiveDescendantId`/`OptionId`
pattern. See `LilyBlazorHeadless`'s own CHANGELOG (root CHANGELOG.md)
...
- **Default icon changed from a Unicode glyph to a bundled SVG.** The
  ...
  (contrast/half-circle design, `stroke="currentColor"`, `stroke-width="1.6"`,
  ...
  stack — no missing-glyph "tofu" risk, no per-glyph optical-scale correction to
  maintain. The public constant **`CircleWithRightHalfBlack`** (◑
  U+25D1) is **removed, not renamed** — there is no longer a
