...
| # | File                                               | Demonstrates                                    |
|---|----------------------------------------------------|-------------------------------------------------|
| 1 | [`Basic.razor`](./Basic.razor)                     | Minimal three-theme picker.                     |
...
| 5 | [`CustomRendering.razor`](./CustomRendering.razor) | `RenderFragment<ThemePickerContext>` — swatch buttons. |
| 6 | [`Preloaded.razor`](./Preloaded.razor)             | Zero-flicker switching via `<HeadContent>` preloads. |
| 7 | [`MultiplePickers.razor`](./MultiplePickers.razor) | Two pickers in one page via distinct `Name`.    |
...
depending on your hosting model; the picker's behaviour is identical
in all three modes.

For a fully-static SSR-only page (no `@rendermode` directive), the
picker renders the markup but cannot mutate the DOM —
