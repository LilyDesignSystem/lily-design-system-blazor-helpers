...
The Blazor-flavoured walk-through of the picker's lifecycle. The
...
mount (server-side render of markup)
  │
  ▼
SSR / prerender produces <fieldset> markup with radios — no DOM mutation, no IJSRuntime call.
  │
  ▼
...
right radio is `checked`. The subsequent `ApplyThemeAsync` writes
the DOM.
...
`RenderFragment<ThemePickerContext>` can call it via
`ctx.SetTheme(slug)`.
...
`OnAfterRenderAsync` never fires. The picker's markup arrives with
...
| --------------- | --------------------------------------------- | --------------------------------------------------- |
| `_initialised = false` | construction                            | Picker is ready to render its markup.               |
