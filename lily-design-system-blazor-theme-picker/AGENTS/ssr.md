...
and the picker does not touch the DOM. The rendered HTML looks like:
...
The picker works in both modes — the server-side cookie strategy
...
The picker stays consistent because:
...
1. The server rendered `Value=""` (no radio checked), but the
...
4. In Blazor Server, confirm the SignalR connection is established
   (no red banner in DevTools). The picker's first interop call
