...
| `ChildContent`       | `RenderFragment<ThemePickerContext>?`| no      | `null` (default radio markup)                    |
| `CssClass`           | `string`                            | no       | `""`                                             |
...
```razor
<ThemePicker Label="Theme" ThemesUrl="/t/" Themes="@(new[]{ "light" })">
...
When no `ChildContent` is supplied, the picker renders the default
radio markup documented in `spec.md §4.2`.
...
```html
<fieldset class="theme-picker {CssClass}" role="radiogroup" aria-label="{Label}">
...
<label class="theme-picker-option">
    <input type="radio" name="{Name}" value="{slug}" checked />
