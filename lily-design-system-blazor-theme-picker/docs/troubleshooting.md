...
**Likely cause.** The component hosting the picker uses a static
(non-interactive) render mode. The picker's lifecycle work runs
...
**Likely cause.** The picker rendered server-side with no `Value`
(no radio checked), then on hydration its lifecycle hook resolved
...
- The picker is interactive (not statically rendered).
...
**Fix.** Designate one picker as the "global" one. For region-
...
- Cookie write happens server-side but the picker writes client-
  side `document.cookie` with different attributes; the next
  request reads the picker's cookie, not the server's. Use a single
  cookie write path.
...
The picker behaves identically in both modes. If you observe a
difference:
...
Use the same `Value`-from-cookie strategy in both cases; the picker
is mode-agnostic.

## "Theme picker mounts twice"
...
the picker in a virtualised list; mount it in a stable layout slot
