...
```razor
<!-- The picker still emits a managed <link>; its href fetch
...
For all four strategies, the picker's `Extension` parameter is the
...
SignalR. The picker's `OnAfterRenderAsync` runs over the SignalR
connection after the page is interactive. If your preload `<link>`
tags fire before the picker's managed `<link>` is created, the
preloaded file is already cached and the picker's `<link>` request
resolves from cache (304 or memory).

The two requests aren't redundant — the preload is a fetch hint,
the picker's `<link>` is the active stylesheet. The browser handles
