...
The picker has no opinion about light vs dark; resolve the media
...
`<link>`s don't collide). Targets other than `<html>` aren't supported
...
The picker reacts via its `Value` parameter on the next render.
...
The two pickers compose without conflict because they own different
