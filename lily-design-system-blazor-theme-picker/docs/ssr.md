...
callback fires and the picker does not touch the DOM. The rendered
HTML looks like:
...
or WebAssembly module loaded), the picker's `OnAfterRenderAsync`
callback runs once:
...
4. The cookie value is forwarded to the picker as a parameter
   (typically a cascading or constructor parameter).
5. The picker mounts with `Value="<cookieTheme>"` — its
...
The picker's first `OnAfterRenderAsync` is then a no-op because the
...
- Use `@rendermode="InteractiveServer"` on the routes that need the
  picker. Static-rendered routes won't run
  `OnAfterRenderAsync(firstRender:true)`.
- For `InteractiveAuto`, the picker behaves the same way; it's the
...
- The picker doesn't care about the render mode of its surrounding
  components; it just needs to run interactively itself.

## What `OnInitializedAsync` vs `OnAfterRenderAsync` means here

- `OnInitializedAsync` runs during prerender. The picker doesn't
...
1. Server rendered `Value=""` (no radio checked), client
   `OnAfterRenderAsync` resolved `Value="dark"` from
   `localStorage`. The first paint sees no `checked`; the
...
render mode), `OnAfterRenderAsync` never fires. The picker:

- Renders its `<fieldset>` markup with the server-supplied `Value`.
...
and the right `<link>` on the server. The picker is purely
...
- **Markup determinism**: assert that
  `RenderComponent<ThemePicker>(...)` produces the expected
  `<fieldset>` for a given `Value` parameter. The same markup is
  what static SSR would emit.
- **No-throw on first render**: catch any exception during the
  first synchronous render. The picker's `[Inject] IJSRuntime JS`
  is loaded lazily — it's not touched until
  `OnAfterRenderAsync(true)`.
- **Integration tests**: run a real Blazor Web App with the picker
  on a static-render page and check the HTTP response body for the
  expected `<fieldset>` markup.
