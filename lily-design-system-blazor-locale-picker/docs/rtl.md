...
2. **DevTools** — Use the element panel to toggle `dir="rtl"` on
   `<html>` directly and observe the layout.
...
### Reactive `dir` via `App.razor`

When pre-resolving the locale server-side, write `dir` in
`App.razor`:
...
server-rendered attribute keeps the SSR markup in sync.

### `:dir()` selector in scoped CSS

`:dir(rtl)` works in Blazor scoped CSS (`MyComponent.razor.css`)
...
### `RightToLeft` in `EditForm`

Blazor's `EditForm` doesn't expose a `RightToLeft` parameter;
the picker's `dir` write covers form layout because everything
inside the document inherits from `<html>`. If you need to scope a
form to LTR even on an RTL page, wrap it in `<div dir="ltr">`.
