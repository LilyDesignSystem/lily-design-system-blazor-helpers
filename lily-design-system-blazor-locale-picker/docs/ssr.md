...
- Picker hydrates already showing the right radio checked because
  `InitialLocale` was hydrated from the cookie.
- User picks `ar`. The endpoint writes the cookie. The picker
...
The picker stays unchanged; the consumer's `App.razor` just gets
...
WebAssembly). The picker flickers (default paints first, then the
...
no Vue-style warning about DOM mismatch. The picker stays
consistent because:
...
1. The server rendered with `Value=""` (no radio checked), but the
   ...
   `localStorage`. The first paint sees no `checked`; the next
...
`HttpContext.Request.Cookies`. The picker doesn't care about the
...
never fires. The picker:

- Renders the `<fieldset>` markup with whatever `Value` the
  consumer supplied.
...
themselves. The picker doesn't break; it just doesn't mutate the
...
- **Compile check** — the picker's source uses no
...
- **Snapshot** — capture the rendered HTML from
  `RenderComponent<LocalePicker>` and assert that the right radio
  has `checked` when `Value` is non-empty.

The picker itself has no SSR-specific code path to test beyond
"the component compiles under static SSR and renders the checked
radio for the seeded `Value`".
