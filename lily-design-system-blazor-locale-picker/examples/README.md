...
- No CSS dependency — the picker is headless. Consumers style
  the `locale-picker`, `locale-picker-option`,
  `locale-picker-list`, `locale-picker-select`, and
  `locale-picker-option-label` class hooks.

| #  | File                                                  | Demonstrates                                                       |
|----|-------------------------------------------------------|--------------------------------------------------------------------|
| 1  | [`01_Radios.razor`](./01_Radios.razor)                | Default `<fieldset role="radiogroup">` rendering.                  |
| 2  | [`02_Select.razor`](./02_Select.razor)                | Native `<select>` dropdown via `ChildContent`.                     |
...
| 9  | [`09_ScopedTarget.razor`](./09_ScopedTarget.razor)    | Multiple per-region pickers, each scoped to its own panel.         |
...
your hosting model; the picker's behaviour is identical in all
three.

For a fully-static SSR-only page (no `@rendermode`), the picker
...
The picker still owns the apply lifecycle (`lang` / `dir` /
