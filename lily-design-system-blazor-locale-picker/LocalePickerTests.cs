// LocalePicker tests — one [Fact] per spec.md §7 acceptance criterion.
...
public class LocalePickerTests : TestContext
{
    ...
    public void Section_7_1_Renders_Fieldset_With_Radiogroup_Role()
    {
        ...
        Assert.Contains("locale-picker", root.GetAttribute("class") ?? "");
    }
    ...
    public void Section_7_2_AriaLabel_Is_Label()
    {
        ...
        Assert.Equal("Choose language", cut.Find("fieldset").GetAttribute("aria-label"));
    }

    // -----------------------------------------------------------------
    // §7.3 — One radio per locale, sharing the supplied Name.
    // -----------------------------------------------------------------
    [Fact]
    public void Section_7_3_One_Radio_Per_Locale_Sharing_Name()
    {
        var cut = RenderComponent<LocalePicker>(p => p
            .Add(x => x.Label, "Language")
            .Add(x => x.Locales, LocalesList)
        ...
        Assert.Equal(LocalesList.Length, radios.Count);
        foreach (var r in radios)
        {
            Assert.Equal("lang", r.GetAttribute("name"));
        }
    }

    // -----------------------------------------------------------------
    // §7.4 — Each radio carries the locale code as its value.
    // -----------------------------------------------------------------
    [Fact]
    public void Section_7_4_Radio_Value_Is_Locale_Code()
    {
        var cut = RenderComponent<LocalePicker>(p => p
            .Add(x => x.Label, "Language")
            .Add(x => x.Locales, LocalesList));
        ...
        {
            Assert.Equal(LocalesList[i], radios[i].GetAttribute("value"));
        }
    }
    ...
    public void Section_7_5_Option_Lang_Is_Bcp47_Hyphen()
    {
        ...
        var labels = cut.FindAll(".locale-picker-option");
    ...
    {
        var script = LocalePicker.BuildApplyScript("en_US", applyDir: true, storageKey: null);
    ...
    {
        var rtl = LocalePicker.BuildApplyScript("ar", applyDir: true, storageKey: null);
        Assert.Contains("setAttribute('dir',\"rtl\")", rtl);

        var ltr = LocalePicker.BuildApplyScript("en", applyDir: true, storageKey: null);
    ...
    // §7.16 — Selecting a different radio updates Value, fires OnChange /
    //         ValueChanged with the consumer-form code (not the BCP 47 tag),
    //         and invokes interop with the new lang/dir.
    [Fact]
    public async Task Section_7_16_Selecting_Radio_Fires_Callbacks_With_Consumer_Form()
    {
        var changed = "";
        var valueChanged = "";
        var cut = RenderComponent<LocalePicker>(p => p
        ...
        var radios = cut.FindAll("input[type=radio]");
        // Click "en_US" (index 1). Callback must receive the consumer
        // form en_US, not the BCP 47 tag en-US.
        await radios[1].ChangeAsync(new() { Value = "en_US" });
    ...
    {
        var script = LocalePicker.BuildApplyScript("zh_Hant_TW", applyDir: true, storageKey: null);
    ...
    {
        var with = LocalePicker.BuildApplyScript("fr", applyDir: true, storageKey: "lily-locale");
        Assert.Contains("localStorage.setItem(\"lily-locale\",\"fr\")", with);

        var without = LocalePicker.BuildApplyScript("fr", applyDir: true, storageKey: null);
    ...
    // -----------------------------------------------------------------
    // §7.22 — Extra attributes spread onto the <fieldset>.
    // -----------------------------------------------------------------
    [Fact]
    public void Section_7_22_AdditionalAttributes_Spread()
    {
        ...
        Assert.Equal("lp", cut.Find("fieldset").GetAttribute("data-testid"));
    }

    // -----------------------------------------------------------------
    // §7.23 — Custom ChildContent receives LocalePickerContext.
    // -----------------------------------------------------------------
    [Fact]
