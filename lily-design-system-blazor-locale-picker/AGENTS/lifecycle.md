...
`RenderFragment<LocalePickerContext>` can call it via
`ctx.SetLocale(code)`.
...
mode), `OnAfterRenderAsync` never fires. The picker's markup
...
`OnAfterRenderAsync(true)`. The picker never re-runs detection
