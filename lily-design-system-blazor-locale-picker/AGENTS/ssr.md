...
The picker runs cleanly under every Blazor 10 hosting model.
...
fires and the picker does not touch the DOM. The rendered HTML
looks like:
...
3. Hydration runs, picker's `OnAfterRenderAsync` resolves
...
WebAssembly. The picker hydrates from `localStorage` like a SPA,
...
The picker doesn't set `CultureInfo` automatically. Wire `OnChange`
if you need it:
...
fires. The picker renders markup but cannot mutate the DOM. Use
the cookie strategy above; the picker is purely presentational
under static SSR.
