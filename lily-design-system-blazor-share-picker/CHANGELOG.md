...
- **Default icon changed from a Unicode glyph to a bundled SVG.** The
  ...
  (outline right arrow (matching https://testingexamples.github.io/) design, `stroke="currentColor"`, `stroke-width="1.6"`,
  ...
  stack — no missing-glyph risk, no per-glyph optical-scale correction to
  maintain. The public constant **`BlackRightwardsArrowhead`** (➤
  U+27A4) is **removed, not renamed** — there is no longer a
