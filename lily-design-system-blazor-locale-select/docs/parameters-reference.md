...
```razor
<LocaleSelect Label="@Localizer["chooseLanguage"]" ... />
```
...
```razor
<LocaleSelect @bind-Value="locale" ... />
...
```razor
<LocaleSelect StorageKey="lily-locale" ... />
```
...
```razor
<LocaleSelect DetectFromNavigator="true" StorageKey="lily-locale" ... />
```
...
The counterpart on ThemeSelect is `DetectFromSystem`, which reads
...
```razor
<LocaleSelect Name="ui-language" ... />
```

Unlike ThemeSelect's `Name`, this one is *only* about form
...
```razor
<LocaleSelect OnChange="OnLocaleChange" ... />
...
## `ChildContent` — optional, `RenderFragment<LocaleSelectContext>?`
...
Supplying `ChildContent` removes the default `.locale-select-icon`
...
```razor
<LocaleSelect CssClass="my-locale-select" ... />
```

The root ends up as `<div class="locale-select my-locale-select" …>`.
...
```razor
<LocaleSelect @ref="localeSelect" Label="Language" Locales="@codes" @bind-Value="locale" />
...
`LocaleName` is the label-resolution counterpart of ThemeSelect's
...
<LocaleSelect @bind-Value="locale" OnChange="OnChange" ... />

<!-- Two-way bind + explicit ValueChanged (rarely needed) -->
<LocaleSelect Value="@locale" ValueChanged="@(v => locale = v)" ... />
```
