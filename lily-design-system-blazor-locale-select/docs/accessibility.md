...
| ---------------------------- | ------------------------------------------------- | ------------------ |
| `div.locale-select`          | none (plain container)                            | Component          |
| `input[type=hidden]`         | `name` — form participation only                  | Component          |
| `button.locale-select-button`| implicit `role="button"`                          | Browser            |
| `button.locale-select-button`| `aria-label="@Label"`                             | Consumer parameter |
| `button.locale-select-button`| `aria-haspopup="listbox"`                         | Component          |
| `button.locale-select-button`| `aria-expanded="true\|false"`                     | Component          |
| `button.locale-select-button`| `aria-controls="{list id}"`                       | Component          |
| `span.locale-select-icon`    | `aria-hidden="true"`                              | Component          |
| `ul.locale-select-list`      | `role="listbox"`, `tabindex="-1"`                 | Component          |
| `ul.locale-select-list`      | `aria-label="@Label"`                             | Consumer parameter |
| `ul.locale-select-list`      | `aria-activedescendant` (only while open)         | Component          |
| `ul.locale-select-list`      | `hidden` while closed                             | Component          |
| `li.locale-select-option`    | `role="option"`, `aria-selected="true\|false"`    | Component          |
| `li.locale-select-option`    | `lang` — BCP 47 hyphen form (WCAG 3.1.2)          | Component          |
| `li.locale-select-option`    | `data-active` on the active option (styling hook) | Component          |
...
presentation so the globe matches ThemeSelect's `◑`). This package
...
robust choice) and/or give `.locale-select-button` a visible
...
```razor
<LocaleSelect Label="Choose a locale" @bind-Value="locale" ... />
...
- **Styling `.locale-select-list` without positioning it.** The package
