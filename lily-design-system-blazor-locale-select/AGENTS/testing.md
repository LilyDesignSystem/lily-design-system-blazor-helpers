...
the select makes when `DetectFromNavigator=true`.
...
public void Section_7_1_Renders_Select()
{
    ...
    Assert.Equal("locale", root.GetAttribute("name"));
...
public void Section_7_5_Per_Option_Lang_Is_Bcp47()
{
    ...
    var options = cut.FindAll("option.locale-select-option");
...
{
    var script = LocaleSelect.BuildApplyScript("ar", applyDir: true, storageKey: null);
...
{
    var script = LocaleSelect.BuildApplyScript("ar", applyDir: false, storageKey: null);
    Assert.Contains("setAttribute('lang','ar')", script);
...
{
    var script = LocaleSelect.BuildApplyScript("en", applyDir: true, storageKey: "lily-locale");
...
await select.ChangeAsync(new() { Value = "ar" });
...
| §7 group        | Test focus                                       |
| --------------- | ------------------------------------------------ |
| 7.1–7.6 markup  | DOM contract: select, options, per-option lang   |
...
| 7.23 children   | ChildContent receives LocaleSelectContext        |
