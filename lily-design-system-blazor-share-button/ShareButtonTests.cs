// ShareButton tests — one [Fact] per spec/index.md §7 acceptance criterion.
//
...
public class ShareButtonTests : TestContext
{
    ...
    /// <summary>
    /// A rendered ShareButton plus the element-reference GUIDs captured from
    ...
    /// </summary>
    private static RefMap MapRefs(IRenderedComponent<ShareButton> cut)
    {
        var found = RefPattern.Matches(cut.Markup)
            .Select(m => (Class: m.Groups["class"].Value, Id: m.Groups["id"].Value))
            .ToList();

        var trigger = found.Single(f => f.Class.Contains("share-button-trigger")).Id;

        // Same order, and the same membership rule, as the canonical Svelte
        // implementation's items(): ".share-button-target, .share-button-copy".
        var items = found
            .Where(f => f.Class.Contains("share-button-target")
                || f.Class.Contains("share-button-copy"))
            .Select(f => f.Id)
            .ToList();
    ...
    private IRenderedComponent<ShareButton> Render(
        Action<ComponentParameterCollectionBuilder<ShareButton>>? extra = null)
        => RenderComponent<ShareButton>(p =>
        {
    ...
    private static void Key(IRenderedComponent<ShareButton> cut, string selector, string key)
        => cut.Find(selector).KeyDown(new KeyboardEventArgs { Key = key });

    private static bool ListHidden(IRenderedComponent<ShareButton> cut)
        => cut.Find("ul.share-button-list").HasAttribute("hidden");

    private static string Status(IRenderedComponent<ShareButton> cut)
        => cut.Find("p.share-button-status").TextContent.Trim();
    ...
    public void Section_7_1_Renders_A_Disclosure_Button_Controlling_A_List()
    {
        ...
        Assert.NotNull(cut.Find("div.share-button"));

        var button = cut.Find("button.share-button-trigger");
        ...
        var list = cut.Find("ul.share-button-list");
    ...
    public void Section_7_1_Button_Renders_Arrow_Glyph_Hidden_From_Assistive_Tech()
    {
        ...
        var icon = cut.Find(".share-button-icon");
        // U+21AA RIGHTWARDS ARROW WITH HOOK.
        Assert.Equal("↪", icon.TextContent.Trim());
        Assert.Equal("↪", ShareButton.RightwardsArrowWithHook);
    ...
    {
        var first = Render().Find("button.share-button-trigger").GetAttribute("aria-controls");
        var second = Render().Find("button.share-button-trigger").GetAttribute("aria-controls");

        Assert.NotEqual(first, second);
        Assert.NotEqual(ShareButton.NextShareButtonId(), ShareButton.NextShareButtonId());
    }
    ...
    public void Section_7_2_List_Is_Hidden_Until_The_Button_Is_Activated()
    {
        ...
        cut.Find("button.share-button-trigger").Click();

        Assert.False(ListHidden(cut));
        Assert.Equal("true", cut.Find("button.share-button-trigger").GetAttribute("aria-expanded"));
    }
    ...
    public void Section_7_3_Destinations_Are_Real_Links_Not_Menuitems()
    {
        ...
        var links = cut.FindAll("a.share-button-target");
    ...
    public void Section_7_3_NewTab_False_Renders_No_Target_Attribute()
    {
        ...
        Assert.Null(cut.Find("a.share-button-target").GetAttribute("target"));
    }
    ...
    {
        var without = Render();
        without.Find("button.share-button-trigger").Click();
        Assert.Empty(without.FindAll("button.share-button-copy"));

        var with = Render(p => p.Add(x => x.CopyLabel, "Copy link"));
        with.Find("button.share-button-trigger").Click();

        var copy = with.Find("button.share-button-copy");
    ...
    public void Section_7_6_Status_Region_Is_Present_Polite_And_Silent_On_Load()
    {
        ...
        var status = cut.Find("p.share-button-status");
    ...
    public void Section_7_7_Copying_Writes_The_Url_And_Fires_OnCopy()
    {
        ...
        cut.Find("button.share-button-trigger").Click();
        cut.Find("button.share-button-copy").Click();

        var call = Assert.Single(CopyCalls());
        // The script the component sent carries the URL it meant to write.
        Assert.Equal(ShareButton.BuildCopyScript(UrlUnderTest), (string)call.Arguments[0]!);
    ...
    public async Task Section_7_10_Absent_Clipboard_Api_Is_A_Failure_Not_A_Crash()
    {
        ...
        JSInterop.Setup<bool>("eval", inv => (string)inv.Arguments[0]! == ShareButton.CanCopyScript)
            .SetResult(false);
        Assert.False(await ShareButton.CanCopyAsync(JSInterop.JSRuntime));
    ...
    public async Task Section_7_11_CanShareNatively_Reflects_Navigator_Share()
    {
        ...
        Assert.False(await ShareButton.CanShareNativelyAsync(JSInterop.JSRuntime));

        JSInterop.Setup<bool>("eval", inv => (string)inv.Arguments[0]! == ShareButton.CanShareScript)
            .SetResult(true);

        Assert.True(await ShareButton.CanShareNativelyAsync(JSInterop.JSRuntime));
    }
    ...
    public void Section_7_15_ArrowDown_On_Closed_Button_Opens_On_First_Item()
    {
        ...
        Key(cut, "button.share-button-trigger", "ArrowDown");
    ...
    public void Section_7_15_ArrowUp_On_Closed_Button_Opens_On_Last_Item()
    {
        ...
        Key(cut, "button.share-button-trigger", "ArrowUp");
    ...
    public void Section_7_16_Arrows_Move_Focus_And_Clamp_At_The_Ends()
    {
        ...
        Key(cut, "ul.share-button-list", "ArrowDown");
        Assert.Equal(refs.Items[1], LastFocusedRefId());

        Key(cut, "ul.share-button-list", "ArrowUp");
        Assert.Equal(refs.Items[0], LastFocusedRefId());
        ...
        Key(cut, "ul.share-button-list", "End");
        Key(cut, "ul.share-button-list", "ArrowDown");
        Assert.Equal(refs.Items[^1], LastFocusedRefId());
    }
    ...
    public void Section_7_16_Home_And_End_Jump_To_First_And_Last()
    {
        ...
        Key(cut, "ul.share-button-list", "Home");
        Assert.Equal(refs.Items[0], LastFocusedRefId());
    }
    ...
    public void Section_7_17_Escape_Closes_And_Returns_Focus_To_The_Button()
    {
        ...
        cut.Find("button.share-button-trigger").Click();
        Key(cut, "ul.share-button-list", "Escape");

        Assert.True(ListHidden(cut));
        Assert.Equal("false", cut.Find("button.share-button-trigger").GetAttribute("aria-expanded"));
    ...
    public void Section_7_17_Tab_Closes_Without_Stealing_Focus_Back()
    {
        ...
        Key(cut, "ul.share-button-list", "Tab");
    ...
    // the root's focusout does the job — the same deviation TextSizeSelect
    ...
    public void Section_7_19_Focus_Leaving_The_Root_Closes_The_List()
    {
        ...
        var root = cut.Find("div.share-button");
    ...
    public void Section_7_20_Explicit_Url_Wins()
    {
        ...
        var href = cut.Find("a.share-button-target").GetAttribute("href")!;
    ...
    {
        RenderFragment<ShareButtonContext> custom_glyph = context => builder =>
        {
        ...
        Assert.Empty(cut.FindAll(".share-button-icon"));
        // And it lives inside the trigger.
        Assert.NotNull(cut.Find("button.share-button-trigger [data-testid='custom']"));
    ...
    public void Section_4_1_CssClass_Merges_And_Attributes_Spread_Onto_Root()
    {
        ...
        var root = cut.Find("div.share-button");
        Assert.Equal("share-button my-share", root.GetAttribute("class"));
