...
`LilyDesignSystem.Blazor.ShareButton.csproj`, or the published
`LilyDesignSystem.Blazor.ShareButton` NuGet package.

```xml
<ProjectReference Include="path/to/LilyDesignSystem.Blazor.ShareButton.csproj" />
```
...
```razor
<ShareButton @ref="share" Label="Share" Targets="@Targets" />
...
`ShareButtonContext` of `{ Open, Url }`:
...
| Member | Purpose |
| ------ | ------- |
| `ShareButton.RightwardsArrowWithHook` | The default glyph, `"↪"` (U+21AA). |
| `ShareButton.NextShareButtonId()` | Mint a stable, prerender-safe id prefix. |
| `ShareButton.CanShareNativelyAsync(js)` | Does this browser have a share sheet? |
| `ShareButton.CanCopyAsync(js)` | Does this browser have an async clipboard? |
