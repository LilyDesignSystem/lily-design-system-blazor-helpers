...
- **`ShareButton`** — a headless share control. A single-glyph button
  ...
  ```html
  <div class="share-button {CssClass}">
...
- **The trigger class is `share-button-trigger`**, not
  ...
  here would read `.share-button-button`. The one deliberate bend in the
  naming convention.
...
- **Static helpers** `RightwardsArrowWithHook`, `NextShareButtonId()`,
...
- **`class` is `CssClass`** (C# keyword); **`children` is
  `ChildContent`**, typed `RenderFragment<ShareButtonContext>`;
