...
- **Every helper in the catalog is renamed to `*-picker`**, resolving a
  name collision: `theme-picker` and `theme-select-option` are catalog
  _components_ in `components.tsv`, and the helper was sharing both the
  slug and the `.theme-picker` class hook with one of them.

  | Was                                          | Becomes                                      |
  | -------------------------------------------- | -------------------------------------------- |
  | `lily-design-system-blazor-theme-select`     | `lily-design-system-blazor-theme-picker`     |
  | `lily-design-system-blazor-locale-select`    | `lily-design-system-blazor-locale-picker`    |
  | `lily-design-system-blazor-text-size-select` | `lily-design-system-blazor-text-size-picker` |
  | `lily-design-system-blazor-share-button`     | `lily-design-system-blazor-share-picker`     |

  NuGet ids follow: `LilyDesignSystem.Blazor.ThemePicker` ->
  `LilyDesignSystem.Blazor.ThemePicker`, and likewise for the other
  three. Components, contexts, class hooks, generated ids and the
  `data-lily-theme-select` link discriminator all move to `picker`.
  Behaviour and API semantics are unchanged.

- **`share-picker` loses its naming exception.** Its trigger class was
...
- **All three helpers** — `theme-picker`, `locale-picker` and now
  `text-size-picker` — are no longer native `<select>` elements. Each is
  ...
  of `<li role="option" aria-selected data-active>`. locale-picker
  options keep their per-locale `lang`; the button and list carry none.
- `text-size-picker` joins the other two in this release, so the three
  ...
  stacks (it falls back to a crude bitmap shape) and means _decrease_
  rather than _size_; `"A"` renders in the page's own font everywhere,
...
- `ChildContent` changes meaning: it now **replaces the glyph inside the
  ...
  listbox semantics (and locale-picker's per-option `lang`) cannot be
  broken by a consumer override. `ThemePickerContext` drops `Themes`,
  `SetTheme` and `Name`; `LocalePickerContext` drops `Locales`,
  `SetLocale`, `Name`, `TagFor` and `IsRtl` — those pure helpers remain
  as statics on the `Locales` class; `TextSizePickerContext` drops
...
- `text-size-picker`'s `TitleCase` internal static is **replaced** by the
  public `TextSizePicker.SizeName`. The new rule no longer strips the
...
- Public glyph constants `ThemePicker.CircleWithRightHalfBlack`,
  `LocalePicker.GlobeWithMeridians`, and
  `TextSizePicker.LatinCapitalLetterA`.
...
- **`TextSizePicker.SizeName(string slug)` — public static.** The single
  ...
  trio alongside `ThemePicker.ThemeName` and `Locales.LocaleName`, so
  consumers rendering their own UI never hand-duplicate the title-casing.
- `text-size-picker` gains `docs/accessibility.md`, `docs/styling.md`,
  and an `examples/` set, matching the other two helpers.

### Not added, deliberately

- **No `DetectFromSystem` on `text-size-picker`.** ThemePicker detects
...
- The status-region pattern survives the rewrite and stays the
  recommendation, but for a different reason: the selection _is_ now
...
- Each package's `docs/accessibility.md` replaces the retired
  ...
  missing depending on the platform. `text-size-picker` notes that its
