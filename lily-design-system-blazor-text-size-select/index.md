...
<p class="text-size-select-status" aria-live="polite">
    Text size: @TextSizeSelect.SizeName(size)
</p>
...
<div class="text-size-select">
  ...
  <ul class="text-size-select-list" id="text-size-select-1-list" role="listbox"
    ...
    <li class="text-size-select-option" role="option" aria-selected="true" data-active>Medium</li>
...
| `ChildContent`        | `RenderFragment<TextSizeSelectContext>?` | no       | the `"A"` glyph                        |
...
- `TextSizeSelect.SizeName(slug)` — the shared title-casing rule
  (`"x-large"` → `"X Large"`). Use it when you render your own labels
  so they match the listbox exactly. Mirrors `ThemeSelect.ThemeName`
...
```razor
<TextSizeSelect Label="Text size" Sizes="sizes" @bind-Value="size">
