...
`lily-design-system-blazor-text-size-select`. Each file is a runnable
...
| 4 | [`CustomRendering.razor`](./CustomRendering.razor) | `RenderFragment<TextSizeSelectContext>` — custom button face (inline SVG, state-aware). |
...
There is no `SystemPreference.razor` here, unlike the theme-select and
locale-select example sets. Browsers expose no "preferred text size"
