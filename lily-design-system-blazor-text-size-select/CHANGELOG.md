# Changelog — TextSizeSelect (Blazor)
...
- **The control is no longer a native `<select>`.** It is now an **icon
  ...
  listbox pattern, matching `ThemeSelect` and `LocaleSelect` so all
  three helpers are one shape:

  ```html
  <div class="text-size-select {CssClass}">
...
- **`ChildContent` changes meaning.** It now **replaces the glyph inside
  ...
  consumer override. `TextSizeSelectContext` drops `Sizes`, `SetSize`
...
- Consumers must update any CSS or test selector targeting
  `select.text-size-select` or `option.text-size-select-option`.
...
- **`TextSizeSelect.LatinCapitalLetterA`** — the public glyph constant.
- Stable, SSR-safe element ids from a monotonic process-wide counter
  (`text-size-select-{n}`) — no randomness and no clock reads.
...
- **No `DetectFromSystem` parameter.** ThemeSelect detects
